...
def train_evaluate_svm(data, label_encoder):
    ...
    npv = recall_score(y_test, y_pred, average='macro')

    plt.figure()
    plot_confusion_matrix(conf_matrix, classes=label_encoder.classes_, title='SVM Multi Classifier Confusion Matri')
    plt.show()

    

    cv_scores = cross_val_score(svm_classifier, X_normalized, y_resampled, cv=5, scoring='f1_weighted', n_jobs=-1)
    #if not os.path.exists("E:/svmmetrics"):
        #os.makedirs("E:/svmmetrics")
    
    #joblib.dump(svm_classifier, "E:/svmmetrics/svm_model.pkl")

    plt.figure()
    plot_confusion_matrix(conf_matrix, classes=label_encoder.classes_, title='SVM Multi Classifier Confusion Matrix')
    plt.savefig("C:/Users/adila/OneDrive/Desktop/confusion_matrix.png")



if __name__ == "__main__":
    data, label_encoder = load_data("E:/Chest X-Ray Classification/CSV/all_features_after_parsing.csv")
